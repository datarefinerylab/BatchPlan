#!/usr/bin/env python3
"""
Main script for extracting floor plans from IFC files
"""
...
from ifc_processor import default_filter, process_storeys, process_levels


def setup_formatters(context, selected_formatters):
    """Setup the requested formatters"""
    formatters = []
...
def load_levels_from_file(level_file):
    ...
    try:
        if context["args"].use_storey:
            ...
            if level_file.exists():
                levels = load_levels_from_file(level_file)
                context["levels"] = levels
                process_levels(context)
            else:
